...
The visual style follows the project guidelines:
- black chart border
- axes start at zero
- horizontal labels, except the Y-axis title
- clear chart title, X-axis title, and Y-axis title
- legend at the top-right
- light blue main marks, light orange outliers for scatter/bubble
- light green outliers for box/violin
...
def _prepare_data(
    ...
    df["per_capita_energy"] = (
        df["primary_energy_consumption"] / df["population"].replace(0, np.nan) * 100
    )

    df["carbon_intensity"] = (fossil / total_mix.replace(0, np.nan) * 100).clip(0, 100)
...
def _guideline_layout(
    ...
    fig.update_layout(
        ...
        plot_bgcolor=WHITE,
        showlegend=True,
        title=dict(text=title, x=0.5, font=dict(size=18, color=BLACK)),
        xaxis_title=x_title,
        yaxis_title=y_title,
        height=height,
        margin=dict(l=85, r=60, t=85, b=80),
    ...
    if x_values is not None:
        fig.update_xaxes(range=_zero_range(x_values))
    else:
        fig.update_xaxes(rangemode="tozero")

    if y_values is not None:
        fig.update_yaxes(range=_zero_range(y_values))
    else:
...
def function_scatter(
    ...
    plot_df = plot_df[(plot_df["gdp"] > 0) & (plot_df["primary_energy_consumption"] > 0)]

    outlier_index = plot_df["primary_energy_consumption"].idxmax()
    normal_df = plot_df.drop(index=outlier_index)
    outlier = plot_df.loc[[outlier_index]]
    ...
    fig.add_trace(
        go.Scatter(
            ...
            mode="markers",
            marker=dict(color=LIGHT_BLUE, size=11, line=dict(color=BLACK, width=1)),
            name="Standard Performance",
            hovertext=normal_df["country"],
            hovertemplate="<b>%{hovertext}</b><br>GDP: %{x:.2f}<br>Energy: %{y:.2f}<extra></extra>",
        )
    )
    fig.add_trace(
        go.Scatter(
            x=outlier["gdp"],
            y=outlier["primary_energy_consumption"],
            mode="markers+text",
            marker=dict(color=LIGHT_ORANGE, size=15, line=dict(color=BLACK, width=1.2)),
            text=outlier["country"],
            textposition="middle right",
            textfont=dict(color=BLACK, size=10),
            name="Performance Outlier",
            hovertemplate="<b>%{text}</b><br>GDP: %{x:.2f}<br>Energy: %{y:.2f}<extra></extra>",
        )
    )

    return _guideline_layout(
        fig,
        title=f"GDP vs Energy Consumption ({selected_year})",
        x_title="GDP",
        y_title="Energy Consumption",
...
def function_bubble(
    ...
    """Bubble Plot: GDP vs Renewable Share, bubble size = population."""
    _, go = _plotly()
    df, selected_year = _prepare_data(data_path, year, include_aggregates)
    plot_df = df.dropna(subset=["gdp", "renewable_share", "population_size"]).copy()
    plot_df = plot_df[(plot_df["gdp"] > 0) & (plot_df["population_size"] > 0)]

    outlier_index = plot_df["population_size"].idxmax()
    normal_df = plot_df.drop(index=outlier_index)
    outlier = plot_df.loc[[outlier_index]]

    max_size = plot_df["population_size"].max()
    normal_sizes = normal_df["population_size"] / max_size * 38 + 8
    outlier_size = outlier["population_size"] / max_size * 48 + 10
    ...
    fig.add_trace(
        go.Scatter(
            ...
            name="Standard Performance",
            hovertext=normal_df["country"],
            hovertemplate="<b>%{hovertext}</b><br>GDP: %{x:.2f}<br>Renewable Share: %{y:.1f}%<extra></extra>",
        )
    )
    fig.add_trace(
        go.Scatter(
            x=outlier["gdp"],
            y=outlier["renewable_share"],
            mode="markers+text",
            marker=dict(
                color=LIGHT_ORANGE,
                size=outlier_size,
            ...
            textfont=dict(color=BLACK, size=10),
            name="Performance Outlier",
            hovertemplate="<b>%{text}</b><br>GDP: %{x:.2f}<br>Renewable Share: %{y:.1f}%<extra></extra>",
        )
    )
...
def function_hist(
    ...
    """Histogram: Distribution of Per Capita Energy Use."""
    _, go = _plotly()
...
def function_box(
    ...
    """Box Plot: Carbon Intensity by Continent."""
    _, go = _plotly()
    from plotly.subplots import make_subplots
...
def function_violin(
    ...
    """Violin Plot: Renewable Share Distribution by Region."""
    _, go = _plotly()
    df, selected_year = _prepare_data(data_path, year, include_aggregates)
